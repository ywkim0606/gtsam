...
/*
 * @file    testSparseDiscreteConditional.cpp
 * @brief   unit tests for SparseDiscreteConditional
 * @author  Yoonwoo Kim
 * @date    May 4, 2023
 */
...
/* ************************************************************************* */
TEST(SparseDiscreteConditional, constructors) {
  DiscreteKey X(0, 2), Y(2, 3), Z(1, 2);  // watch ordering !

  SparseDiscreteConditional actual(X | Y = "1/1 2/3 1/4");
  ...
  TableFactor f1(X & Y, "0.5 0.4 0.2 0.5 0.6 0.8");
  SparseDiscreteConditional expected1(1, f1);
  EXPECT(assert_equal(expected1, actual, 1e-9));

  TableFactor f2(
      X & Y & Z, "0.2 0.5 0.3 0.6 0.4 0.7 0.25 0.55 0.35 0.65 0.45 0.75");
  SparseDiscreteConditional actual2(1, f2);
  TableFactor expected2 = f2 / *f2.sum(1);
  EXPECT(assert_equal(expected2, static_cast<TableFactor>(actual2)));
}

/* ************************************************************************* */
TEST(SparseDiscreteConditional, constructors_alt_interface) {
  DiscreteKey X(0, 2), Y(2, 3), Z(1, 2);  // watch ordering !

  const Signature::Row r1{1, 1}, r2{2, 3}, r3{1, 4};
  const Signature::Table table{r1, r2, r3};
  SparseDiscreteConditional actual1(X, {Y}, table);
...
/* ************************************************************************* */
TEST(SparseDiscreteConditional, constructors2) {
  DiscreteKey C(0, 2), B(1, 2);
  Signature signature((C | B) = "4/1 3/1");
  SparseDiscreteConditional actual(signature);

  TableFactor expected(C & B, "0.8 0.75 0.2 0.25");
  EXPECT(assert_equal(expected, static_cast<TableFactor>(actual)));
}

/* ************************************************************************* */
TEST(SparseDiscreteConditional, constructors3) {
  DiscreteKey C(0, 2), B(1, 2), A(2, 2);
  Signature signature((C | B, A) = "4/1 1/1 1/1 1/4");
  SparseDiscreteConditional actual(signature);

  TableFactor expected(C & B & A, "0.8 0.5 0.5 0.2 0.2 0.5 0.5 0.8");
  EXPECT(assert_equal(expected, static_cast<TableFactor>(actual)));
}

/* ****************************************************************************/
// Test evaluate for a discrete Prior P(Asia).
TEST(SparseDiscreteConditional, PriorProbability) {
  constexpr Key asiaKey = 0;
  const DiscreteKey Asia(asiaKey, 2);
  SparseDiscreteConditional dc(Asia, "4/6");
  DiscreteValues values{{asiaKey, 0}};
  EXPECT_DOUBLES_EQUAL(0.4, dc.evaluate(values), 1e-9);
  EXPECT(SparseDiscreteConditional::CheckInvariants(dc, values));
}

/* ************************************************************************* */
// Check that error, logProbability, evaluate all work as expected.
TEST(SparseDiscreteConditional, probability) {
  DiscreteKey C(2, 2), D(4, 2), E(3, 2);
  SparseDiscreteConditional C_given_DE((C | D, E) = "4/1 1/1 1/1 1/4");
  ...
  EXPECT(SparseDiscreteConditional::CheckInvariants(C_given_DE, given));
}

/* ************************************************************************* */
// Check calculation of joint P(A,B)
TEST(SparseDiscreteConditional, Multiply) {
  DiscreteKey A(1, 2), B(0, 2);
  SparseDiscreteConditional conditional(A | B = "1/2 2/1");
  SparseDiscreteConditional prior(B % "1/2");

  // The expected factor
  TableFactor f(A & B, "1 4 2 2");
  SparseDiscreteConditional expected(2, f);
...
TEST(SparseDiscreteConditional, Multiply2) {
  DiscreteKey A(0, 2), B(1, 2), C(2, 2);
  SparseDiscreteConditional A_given_B(A | B = "1/3 3/1");
  SparseDiscreteConditional B_given_C(B | C = "1/3 3/1");
...
TEST(SparseDiscreteConditional, Multiply3) {
...
TEST(SparseDiscreteConditional, Multiply4) {
  DiscreteKey A(0, 2), B(1, 2), C(2, 2), D(4, 2), E(3, 2);
  SparseDiscreteConditional A_given_B(A | B = "1/3 3/1");
  SparseDiscreteConditional B_given_D(B | D = "1/3 3/1");
  SparseDiscreteConditional AB_given_D = A_given_B * B_given_D;
  SparseDiscreteConditional C_given_DE((C | D, E) = "4/1 1/1 1/1 1/4");
...
TEST(SparseDiscreteConditional, marginals) {
  DiscreteKey A(1, 2), B(0, 2);
  SparseDiscreteConditional conditional(A | B = "1/2 2/1");
  SparseDiscreteConditional prior(B % "1/2");
  SparseDiscreteConditional pAB = prior * conditional;

  // P(A=0) = P(A=0|B=0)P(B=0) + P(A=0|B=1)P(B=1) = 1*1 + 2*2 = 5
  // P(A=1) = P(A=1|B=0)P(B=0) + P(A=1|B=1)P(B=1) = 2*1 + 1*2 = 4
  SparseDiscreteConditional actualA = pAB.marginal(A.first);
  SparseDiscreteConditional pA(A % "5/4");
  EXPECT(assert_equal(pA, actualA));
  EXPECT(actualA.frontals() == KeyVector{1});
  EXPECT_LONGS_EQUAL(0, actualA.nrParents());

  SparseDiscreteConditional actualB = pAB.marginal(B.first);
...
TEST(SparseDiscreteConditional, marginals2) {
  DiscreteKey A(0, 2), B(1, 2);  // changing keys need to make pruning happen!
  SparseDiscreteConditional conditional(A | B = "2/2 3/1");
  SparseDiscreteConditional prior(B % "1/2");
  SparseDiscreteConditional pAB = prior * conditional;
  // P(A=0) = P(A=0|B=0)P(B=0) + P(A=0|B=1)P(B=1) = 2*1 + 3*2 = 8
  // P(A=1) = P(A=1|B=0)P(B=0) + P(A=1|B=1)P(B=1) = 2*1 + 1*2 = 4
  SparseDiscreteConditional actualA = pAB.marginal(A.first);
  SparseDiscreteConditional pA(A % "8/4");
  EXPECT(assert_equal(pA, actualA));
...
/* ************************************************************************* */
TEST(SparseDiscreteConditional, likelihood) {
  DiscreteKey X(0, 2), Y(1, 3);
  SparseDiscreteConditional conditional(X | Y = "2/8 4/6 5/5");

  auto actual0 = conditional.likelihood(0);
  TableFactor expected0(Y, "0.2 0.4 0.5");
  EXPECT(assert_equal(expected0, *actual0, 1e-9));

  auto actual1 = conditional.likelihood(1);
  TableFactor expected1(Y, "0.8 0.6 0.5");
  EXPECT(assert_equal(expected1, *actual1, 1e-9));
}

/* ************************************************************************* */
// Check choose on P(C|D,E)
TEST(SparseDiscreteConditional, choose) {
  DiscreteKey C(2, 2), D(4, 2), E(3, 2);
  SparseDiscreteConditional C_given_DE((C | D, E) = "4/1 1/1 1/1 1/4");
  ...
  SparseDiscreteConditional expected2(C | E = "1/1 1/4");
  ...
  SparseDiscreteConditional expected3(C % "1/1");
  EXPECT(assert_equal(expected3, *actual3, 1e-9));
}

/* ************************************************************************* */
// Check markdown representation looks as expected, no parents.
TEST(SparseDiscreteConditional, markdown_prior) {
  DiscreteKey A(Symbol('x', 1), 3);
  SparseDiscreteConditional conditional(A % "1/2/2");
  string expected =
      " *P(x1):*\n\n"
      "|x1|value|\n"
      "|:-:|:-:|\n"
...
TEST(SparseDiscreteConditional, markdown_prior_names) {
  Symbol x1('x', 1);
  DiscreteKey A(x1, 3);
  SparseDiscreteConditional conditional(A % "1/2/2");
  string expected =
      ...
      "|A1|0.4|\n"
      "|A2|0.4|\n";
  TableFactor::Names names{{x1, {"A0", "A1", "A2"}}};
...
TEST(SparseDiscreteConditional, markdown_multivalued) {
  DiscreteKey A(Symbol('a', 1), 3), B(Symbol('b', 1), 5);
  SparseDiscreteConditional conditional(
...
TEST(SparseDiscreteConditional, markdown) {
  DiscreteKey A(2, 2), B(1, 2), C(0, 3);
  SparseDiscreteConditional conditional(A, {B, C}, "0/1 1/3  1/1 3/1  0/1 1/0");
  string expected =
      " *P(A|B,C):*\n\n"
      "|*B*|*C*|T|F|\n"
      "|:-:|:-:|:-:|:-:|\n"
...
TEST(SparseDiscreteConditional, html) {
  DiscreteKey A(2, 2), B(1, 2), C(0, 3);
  SparseDiscreteConditional conditional(A, {B, C}, "0/1 1/3  1/1 3/1  0/1 1/0");
  string expected =
      "<div>\n"
      "<p>  <i>P(A|B,C):</i></p>\n"
      "<table class='SparseDiscreteConditional'>\n"
      "  <thead>\n"
